...
# 운영자 ID 지정 (실제 AI 호출/DM/운영 명령)
OPEN_ACCESS=false
ALLOWLIST=discord:<your_discord_user_id>

# 공개 채널에서 일반 사용자에게 AI 호출을 열려면 채널 단위로 허용
CHANNEL_ALLOWLIST=discord:<trusted_channel_id>
...
Windows 서비스처럼 `.env` 평문 파일을 쓰지 않는 운영 방식에서는 `ORKA_DISABLE_DOTENV=true`를 서비스 환경에 설정하면 자동 로드를 끌 수 있습니다.
...
운영 기본값에서는 사용자가 operator이거나 해당 채널이 `CHANNEL_ALLOWLIST`에 있어야 실제 AI CLI가 실행됩니다.
...
- 운영 기본값에서는 사용자가 operator이거나 해당 채팅이 `CHANNEL_ALLOWLIST`에 있어야 실제 AI CLI가 실행됩니다.
...
게이트웨이는 **누가 operator 명령과 실제 AI CLI 호출을 실행할 수 있는지**를 제어합니다. 기본값에서는 공개 채널의 일반 메시지도 operator 또는 `CHANNEL_ALLOWLIST`로 승인된 채널에서만 AI CLI를 호출합니다. 또한 **DM(Discord DM / Telegram 개인 대화)은 allowlist된 operator만 사용할 수 있습니다.**
...
- 서버/그룹 채널 메시지는 operator, `CHANNEL_ALLOWLIST`, 또는 명시적 `PUBLIC_CHAT=true`일 때만 AI CLI를 호출합니다.
...
CHANNEL_ALLOWLIST=discord:<trusted_channel_id>   # AI 호출을 허용할 공유 채널
PUBLIC_CHAT=false

MAX_CONCURRENT_TASKS=4           # 동시 4건까지 처리
RATE_LIMIT_MAX_REQUESTS=5        # 사용자 scope당 분당 5건 제한
...
| `HEALTH_BEARER_TOKEN` | (빈값) | 설정하면 `/healthz`, `/readyz`, `/metrics` 요청에 `Authorization: Bearer <token>`이 필요합니다. `HEALTH_BIND`를 loopback 밖으로 열 때 설정하세요. |
...
| `CHANNEL_ALLOWLIST` | (빈값) | 비-operator의 공개 채널 AI 호출을 허용할 채널 목록. 예: `discord:<channel_id>,telegram:<chat_id>`. |
| `PUBLIC_CHAT` | `false` | `true`이면 비-operator도 봇이 참여한 모든 공개 채널에서 AI CLI를 호출할 수 있습니다. 운영에서는 채널 allowlist를 우선하세요. |
...
| `RATE_LIMIT_MAX_REQUESTS` | `5` | 윈도우 내 scope(채널+채팅방+사용자)당 최대 요청 수. `0`이면 제한 없음. 예: `10`이면 60초간 같은 사용자 scope에서 최대 10건. |
